...
public class ClockView extends View {
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        canvas.save();
        int Degree = calendar.get(Calendar.HOUR)*360/12+calendar.get(Calendar.MINUTE)*360/12/60-90;
        double Radians = Math.toRadians(Degree);
        float startX = (float) (centerX);
        float startY = (float) (centerY);
        float stopX = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO *Math.cos(Radians));
        float stopY = (float) (centerY + radius * HOUR_NEEDLE_LENGTH_RATIO*Math.sin(Radians));
        needlePaint.setStrokeWidth(HOUR_NEEDLE_WIDTH);
        canvas.drawLine(startX,startY,stopX,stopY,needlePaint);
        canvas.restore();

        canvas.save();
        Degree = calendar.get(Calendar.MINUTE)*360/60-90;
        Radians = Math.toRadians(Degree);
        startX = (float) (centerX);
        startY = (float) (centerY);
        stopX = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO *Math.cos(Radians));
        stopY = (float) (centerY + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(Radians));
        needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        canvas.drawLine(startX,startY,stopX,stopY,needlePaint);
        canvas.restore();

        canvas.save();
        Degree = calendar.get(Calendar.SECOND)*360/60-90;
        Radians = Math.toRadians(Degree);
        startX = (float) (centerX);
        startY = (float) (centerY);
        stopX = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO *Math.cos(Radians));
        stopY = (float) (centerY + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.sin(Radians));
