...
public class MainActivity extends AppCompatActivity {
    ...
    private void getData(String studentId){
        new Thread(new Runnable() {
            @Override
            public void run() {
                List<Message> messages = GetMessageFromRemote(studentId);
                if(messages !=null) {
    ...
    private List<Message> GetMessageFromRemote(String studentId)
    {
        String urlStr;
        if(studentId==null) urlStr = Constants.BASE_URL+"messages";
        else urlStr = Constants.BASE_URL+"messages?student_id="+studentId;
        MessageListResponse messages = null;
        try {
